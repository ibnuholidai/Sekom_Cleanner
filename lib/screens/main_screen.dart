...
class _MainScreenState extends State<MainScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _checkFolderSizes() async {
    try {
      List<FolderInfo> folderInfos = await SystemService.getFolderSizes();
      setState(() {
  ...
  Future<void> _checkFolderSizesFast() async {
    try {
      List<FolderInfo> folderInfos = await SystemService.getFolderSizesUltraFast(timeout: const Duration(seconds: 6));
      setState(() {
        _folderInfos = folderInfos;
      });
    } catch (e) {
